...
final class LlamaSuggestionEngineCancellationTests: XCTestCase {
    ...
    func test_runtimeCancelledError_doesNotResetCache_andThrowsCancelled() async {
        ...
        let engine = LlamaSuggestionEngine(runtimeManager: runtime, suggestionSettings: makeSettings())
    ...
    private func makeSettings() -> SuggestionSettingsModel {
        let defaults = UserDefaults(suiteName: "CotabbyTests.\(UUID().uuidString)")!
...
private final class FakeLlamaRuntime: LlamaRuntimeGenerating {
    ...
    func generateTree(
        ...
        let text = try generateResult.get()
        return TreeDecodeResult(
            candidates: [
                TreeDecodeCandidate(text: text, tokenCount: text.count, latency: 0, branchIndex: 0)
            ],
            totalLatency: 0
        )
    }
