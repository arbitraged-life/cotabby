...
final class InputMonitor {
    ...
    private func classify(keyEvent: InputMonitorKeyEvent, recognizesAcceptance: Bool = true) -> CapturedInputEvent {
        ...
        if flags.contains(.maskAlternate), !flags.contains(.maskCommand) {
            if keyCode == 30 {
                return CapturedInputEvent(kind: .cycleNext, keyCode: keyCode, characters: "", flags: flags)
            }
            if keyCode == 33 {
                return CapturedInputEvent(kind: .cyclePrevious, keyCode: keyCode, characters: "", flags: flags)
            }
        }
